...
const AppartmentPage = ({ route }) => {
  ...
  useEffect(() => {
    const fetchApartmentPosts = async () => {
      setIsLoading(true);
      await new Promise((resolve) => setTimeout(resolve, 2000));

      let posts = apartmentPosts;

      if (userId === "659047e3b7c761390fc26149") {
        posts = posts.slice().sort((a, b) => a.rent - b.rent);
      } else if (userId === "65e30cd149ebc9f65f700e2d") {
        posts = posts.slice().sort((a, b) => {
          if (a.area === "Clifton" || a.area === "DHA") return -1;
          if (b.area === "Clifton" || b.area === "DHA") return 1;
          return 0;
        });
      } else {
        posts = posts.slice();
    ...
    fetchApartmentPosts();
  }, [searchText, sortOption, userId, apartmentPosts]);
